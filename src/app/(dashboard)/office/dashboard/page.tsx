...
export default function OfficeDashboard() {
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 font-outfit">Office Dashboard</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex items-center justify-between p-4 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-md bg-yellow-50 text-yellow-600 border border-yellow-100">
                  Pending
                </span>
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div className="space-y-3">
            <Link href="/office/inquiries" className="w-full flex items-center justify-between p-4 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors shadow-lg shadow-blue-600/20">
              New Inquiry
              <UserPlus className="h-5 w-5" />
            </Link>
            <button className="w-full flex items-center justify-between p-4 rounded-xl border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition-colors">
