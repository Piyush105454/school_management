...
export default function TeacherIncidentClient({ teacherId }: { teacherId: string }) {
  ...
  const loadIncidents = async () => {
    setLoading(true);
    const res = await getTeacherIncidentsAction(teacherId);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic flex items-center gap-3">
            ...
            <span>Tagged {itemTypeLabel}s</span>
          </h1>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
            {activeTab === "INCIDENT" && "View logged security and infrastructure incidents you are tagged in"}
            {activeTab === "COMPLAIN" && "View and monitor student complaints you are tagged in"}
                        ...
                        <div className="flex flex-wrap gap-2 pt-1">
                          {item.student && (
                            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded text-[9px] font-bold">
                              ...
                              Student: {item.student.name} {item.class ? `(${item.class.name})` : ""}
                            </span>
                          )}
