...
export function GlobalInstituteFilter() {
  ...
  // Debug log
  React.useEffect(() => {
    console.log("GlobalInstituteFilter - Institutes:", institutes, "Loading:", loading, "Selected:", selectedInstitute);
  }, [institutes, loading, selectedInstitute]);

  // Only show for ADMIN (OFFICE) or PRINCIPAL role - MUST be after all hooks
  if (session?.user?.role !== "OFFICE" && session?.user?.role !== "PRINCIPAL" && session?.user?.role !== "ADMIN") return null;

  return (
    <div className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)}
        disabled={institutes.length === 0 && !loading}
        className={`flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-full text-sm font-bold text-slate-700 hover:border-blue-500 hover:bg-blue-50/30 transition-all shadow-sm active:scale-95 ${institutes.length === 0 && !loading ? 'opacity-60 cursor-not-allowed' : ''}`}
      >
        <div className="h-6 w-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center">
          <School className="h-3.5 w-3.5" />
        </div>
        <span className="hidden sm:inline text-xs">
          {loading ? "Loading..." : (selectedInstitute === "ALL" ? "Global Institute Filter" : selectedInstitute)}
        </span>
            ...
            <div className="max-h-64 overflow-y-auto p-1.5">
              {loading ? (
                ...
                <div className="py-4 px-3 text-center text-xs text-slate-500">
                  No institutes found in database
                </div>
              ) : (
                <>
                  <button
