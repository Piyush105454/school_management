...
export function EntranceTestTable({ applicants, teachers = [] }: EntranceTestTableProps) {
  const searchParams = useSearchParams();
  const expandId = searchParams.get("expand");
  const [expandedId, setExpandedId] = useState<string | null>(expandId);

  const toggleExpand = (id: string, e: React.MouseEvent) => {
    // Prevent expansion when clicking inside an already expanded area if it has its own logic
    // but here we want the row click to toggle
    setExpandedId(expandedId === id ? null : id);
  };

  const getStatusBadge = (status: string) => {
    const configs: Record<string, { label: string, classes: string }> = {
      "PASS": { label: "Passed", classes: "bg-emerald-50 text-emerald-700 border-emerald-100" },
      "FAIL": { label: "Failed", classes: "bg-red-50 text-red-700 border-red-100" },
        ...
        <table className="w-full text-left border-separate border-spacing-0">
          <thead className="bg-slate-50/50 text-slate-500 text-[10px] font-black uppercase tracking-[0.15em]">
            <tr>
              ...
              <th className="px-6 py-5 border-b border-slate-100">Test Schedule</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {applicants.map((applicant) => {
              ...
              const test = applicant.entranceTest;
              const hasTest = !!test?.testDate;
                      ...
                      {hasTest ? (
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700">
                             <Calendar size={12} className="text-blue-500" /> {formatDate(test.testDate)}
                          </div>
                          <div className="flex items-center gap-1.5 text-[10px] font-medium text-slate-400">
                             <Clock size={12} /> {formatTime(test.testTime)}
                          </div>
                        </div>
                      ) : (
                    ...
                    <tr>
                      <td colSpan={6} className="p-0 border-none">
                        <div className="bg-slate-50/50 p-6 md:p-10 border-b border-t border-slate-100">
                           <div className="max-w-5xl mx-auto animate-in slide-in-from-top-4 duration-300">
